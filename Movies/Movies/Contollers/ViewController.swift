...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if movies[indexPath.row].rating! >= 9 {
            cell.ratingView.backgroundColor = UIColor.green
        } else if movies[indexPath.row].rating! <= 9 && movies[indexPath.row].rating! >= 5 {
            cell.ratingView.backgroundColor = UIColor.orange
...
extension ViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let vc = storyboard?.instantiateViewController(withIdentifier: "CastMemberViewController") as! ViewController
        if indexPath.row == 0 {
